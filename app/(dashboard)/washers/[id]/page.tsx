...
import { approveUser, getErrorMessage, getWasherById, rejectUser } from "@/lib/api";

export default function WasherDetailsPage({ params }: { params: { id: string } }) {
  const router = useRouter();
  const queryClient = useQueryClient();
  const { data, isLoading } = useQuery({
    queryKey: ["washer", params.id],
    queryFn: () => getWasherById(params.id),
  });

  const approveMutation = useMutation({
    mutationFn: () => approveUser(params.id),
    onSuccess: () => {
      toast.success("Washer updated");
      queryClient.invalidateQueries({ queryKey: ["washer", params.id] });
    },
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <button className="inline-flex items-center gap-2 text-3xl font-semibold" onClick={() => router.back()}>
          <ArrowLeft className="h-5 w-5" /> Washer Details
        </button>
      </div>
        ...
        <>
          ...
          <Card className="bg-[#f4f6f8]">
            <CardContent className="grid gap-4 p-6 md:grid-cols-2">
              ...
              <div>
                <p className="mb-2 text-lg font-medium">Passport / Driving license Photo</p>
                <div className="flex h-64 items-center justify-center rounded-xl border border-dashed border-[#c7cbd1] text-lg text-[#6b7280]">
                  Photo showing here
                </div>
              </div>
              <div>
                <p className="mb-2 text-lg font-medium">ID Photo</p>
                <div className="flex h-64 items-center justify-center rounded-xl border border-dashed border-[#c7cbd1] text-lg text-[#6b7280]">
                  Photo showing here
                </div>
              </div>
